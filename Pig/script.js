...
function Chicken() {
  ...
  // Define the base size for the pig
  ...
  const textureLoader = new THREE.TextureLoader();
  const bodyTexture = textureLoader.load("assets/pigs.png"); // Load pig.jpg texture

  // Create the body of the pig with the texture
  const bodyMaterial = new THREE.MeshPhongMaterial({
    map: bodyTexture, // Use the loaded texture for the body
    flatShading: true,
  });
  ...
  // Create the snout of the pig with a darker pink color
  const snoutMaterial = new THREE.MeshPhongMaterial({
    color: 0xff69b4, // Darker pink for the snout
    flatShading: true,
  });
  const snout = new THREE.Mesh(
    new THREE.CylinderBufferGeometry(5 * zoom, 5 * zoom, 10 * zoom, 32),
    snoutMaterial
  );
  snout.position.set(0, 0, 15 * zoom); // Position the snout in front of the body
  snout.rotation.x = Math.PI / 2; // Rotate the snout to face forward
  snout.castShadow = true;
  snout.receiveShadow = true;
  pig.add(snout);

  // Create the ears of the pig
  const earMaterial = new THREE.MeshPhongMaterial({
    color: 0xffc0cb, // Same pink color as the body
    flatShading: true,
  });
  const earGeometry = new THREE.BoxBufferGeometry(5 * zoom, 5 * zoom, 1 * zoom);
  const leftEar = new THREE.Mesh(earGeometry, earMaterial);
  leftEar.position.set(-7 * zoom, 0, 20 * zoom); // Position the left ear
  leftEar.rotation.z = Math.PI / 4; // Rotate the ear slightly
  leftEar.castShadow = true;
  leftEar.receiveShadow = true;
  pig.add(leftEar);

  const rightEar = new THREE.Mesh(earGeometry, earMaterial);
  rightEar.position.set(7 * zoom, 0, 20 * zoom); // Position the right ear
  rightEar.rotation.z = -Math.PI / 4; // Rotate the ear slightly
  rightEar.castShadow = true;
  rightEar.receiveShadow = true;
  pig.add(rightEar);

  // Create the tail of the pig
  const tailMaterial = new THREE.MeshPhongMaterial({
    color: 0xffc0cb, // Same pink color as the body
    flatShading: true,
  });
  const tail = new THREE.Mesh(
    new THREE.TorusBufferGeometry(3 * zoom, 1 * zoom, 16, 100, Math.PI),
    tailMaterial
  );
  tail.position.set(0, -10 * zoom, 10 * zoom); // Position the tail at the back of the body
  tail.rotation.x = Math.PI / 2; // Rotate the tail to face backward
