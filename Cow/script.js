...
function Chicken() {
  ...
  // Define the base size for the cow-like shape
  const baseSize = chickenSize * zoom;
  const heightFactor = 1.5; // Adjusted factor to make the body smaller

  // Load cow texture
  const cowTexture = new THREE.TextureLoader().load("assets/cow.jpg"); // Ensure cow.jpg is in the assets folder

  // Create the body of the cow (making it smaller and still fat)
  ...
  // Create legs for the cow (remain unchanged)
  const legMaterial = new THREE.MeshPhongMaterial({ color: 0x4b4b4b }); // Dark color for legs
  const legWidth = baseSize * 0.3;
  const legHeight = baseSize * 0.8; // Taller legs
